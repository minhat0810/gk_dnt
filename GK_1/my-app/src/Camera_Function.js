...
function Camera_Function() {
  ...
  return (
    <div>
      <p>Trạng thái mạng: <strong>{networkStatus}</strong></p>
      <button onClick={takePhoto} style={{ padding: '10px 20px', fontSize: '16px', marginRight: '10px' }}>
        Chụp Ảnh
      </button>
      <button onClick={accessPhotoLibrary} style={{ padding: '10px 20px', fontSize: '16px' }}>
        Truy cập Thư viện Ảnh
      </button>
      {imageUrl && (
        <div style={{ marginTop: '20px' }}>
          <img src={imageUrl} alt="Ảnh đã lưu" style={{ maxWidth: '100%', height: 'auto', border: '1px solid #ccc' }} />
        </div>
      )}
    </div>
  );
